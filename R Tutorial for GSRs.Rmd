...
csv %>% # Call the data set with pipe function to add subsequent action.
  ...
  mutate(count = n()) %>% # Create a new variable "count" which counts the total number of rows in each group.
...
csv %>% # Again call the original data set with pipe function.
  ...
  mutate(avg_One.Year.Graduate.Count = mean(One.Year.Graduate.Count, na.rm = TRUE)) %>% # Create the mean variable.
...
csv_county.name <- csv %>% # Call the data set with pipe function and save in a new data object.
  select(County.Name) %>% # Pull out the needed column.
  sample_n(10) %>% # Get a random sample of 10 entries.
  mutate(Random.Number = sample(1:100, n(), replace = FALSE)) # Add a column with random number between 1 and 100 for each county.
```

```{r Pipe Function - Example 4}
# Now that we have saved this new object, say we want to join it against the original data set.
csv %>% # Again, call the data set with pipe function.
  left_join(csv_county.name, by = "County.Name") %>% # Left join the two data sets by county name.
